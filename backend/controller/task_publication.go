...
func GetTaskPublications(c *gin.Context) {
	...
	err := database.DB.Table("task_publications").
		Select("genres.id as genre_id, genres.name as genre_name, genres.display_order as genre_display_order, seasons.id as season_id, seasons.number as season_number, teams.id as team_id, teams.name as team_name, task_publications.is_published").
	...
	c.JSON(http.StatusOK, gin.H{"taskPublications": taskPublications})
}
