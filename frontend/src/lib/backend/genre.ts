...
import { GenreWithReference } from "./types/genre";
...
export async function getGenre(genreId: number): Promise<Genre> {
  const response = await fetch(`${process.env.BACKEND_URL}/genres/${genreId}`, { next: { revalidate: 0 } });
  const { genre } = await response.json();
  return genre;
}

export async function createGenre(genreName: string, displayOrder: number): Promise<void> {
...
export async function updateGenre(genreId: number, genreName: string, displayOrder: number): Promise<void> {
